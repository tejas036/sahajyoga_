...
$backToForm = 'https://www.sahajayogabharat.org/sahajayoga_profile/result.php';
//echo $id;
$Date =  date("Y-m-d");
...
switch ($table_id) {
  case 'transactions':
    $sql = "SELECT * FROM transactions WHERE Transaction_id = '$id' AND status='Success'";
    break;
  case 'event_registration':
    $sql = "SELECT * FROM event_registration WHERE Transaction_id = '$id'AND status='Success'";
    break;
  case 'event_registration_rahuri':
    $sql = "SELECT * FROM event_registration_rahuri WHERE Transaction_id = '$id' AND status='Success'";
    break;
  default:
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    switch ($table_id) {
      ...
      case 'event_registration':
        ...
        break;
      case 'event_registration_rahuri':
        $countQuery = "SELECT count(*) as count,participant_type FROM `participants_rahuri` where event_registration_id=$eventRegId group by participant_type";
        $resultCountQuery = $conn->query($countQuery);
        ...
        $imageQuery = "SELECT image FROM `participants_rahuri` where event_registration_id=$eventRegId limit 1";
        $resultImageQuery = $conn->query($imageQuery);
        if ($resultImageQuery->num_rows > 0) {
          while ($row = $resultImageQuery->fetch_assoc()) {
            $image = $row['image'];
        ...
        $couponQuery = "SELECT coupon_number,name,gender,participant_type FROM `participants_rahuri` where event_registration_id=$eventRegId";
        $resultCouponQuery = $conn->query($couponQuery);
        if ($resultCouponQuery->num_rows > 0) {
          while ($row = $resultCouponQuery->fetch_assoc()) {
              ...
              <div class="heading">
                <div class="row">
                  <div class="col-md-1 text-center" style="padding: 5px">
                    <img class="print-margin-left" src="../gpseminar/success.gif" alt="" srcset="" style="border-radius: 99022px; height: 40px; width: 50px; margin-left: 0px;" />
                  </div>
                  <div
