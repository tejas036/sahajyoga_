				...
				<?php
				   ...
				   $yesterdayDate = "2023-10-20";
					...
					if(!empty($_POST["search"])) {
					 ...
					 $sql = "SELECT * from event_registration WHERE Status = 'Success' and paymenttype ='foreign' and Transaction_start_time between '$yesterdayDate' and '$todayDate'  $cpeventtype    ORDER BY event_registration_id desc";
					}
		...
		<table class='table table-bordered hide' id="ex" width="100%" cellspacing="0">
         ...
         <tr>
		  ...
		  <?php 
				...
				if($towards == 'Foreign Sahaja Yogis paying via Indian Bank account')
                 {
				echo $adultcount=$row["count_adult"] * 10000;
				 }
		  ...
		  <?php 
				...
				if($towards == 'Foreign Sahaja Yogis paying via Indian Bank account')
                 {
				echo $adultcount=$row["count_yuva"] * 7500;
				 }
		  ...
		  <?php 
				...
				if($towards == 'Foreign Sahaja Yogis paying via Indian Bank account')
                 {
				echo $adultcount=$row["count_child"] * 7500;
				 }
			...
			<tr>
				...
				<?PHP 
				  ...
				  $TTFemaleresult1 = $conn->query($TTFemale1)->fetch_assoc();
					...
					$adulttotal1 = $TTFemaleresult1['sum(count_adult)'] * 10000;
				...
				<?PHP 
						...
						$yuvatotal1 = $TTFemaleresult1['sum(count_yuva)'] * 7500;
				...
				$TTFemaleresult = $conn->query($TTFemale)->fetch_assoc();			
						...
						$childtotal1 = $TTFemaleresult1['sum(count_child)'] * 7500;
