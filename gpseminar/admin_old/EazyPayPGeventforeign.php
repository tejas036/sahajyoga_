...
{
  $adulttotal=$adultcount * 10000;
  $yuvatotal=$yuvacount * 7500;
  $childtotal= $childcount * 7500;
  $gtotal=$adulttotal+$yuvatotal+$childtotal;
}



if($amountfromget == $gtotal && $eventId != '' && $payerFirstName != '' && $payerLastName != ''  && $payerEmail != '' && $city != '' && $trans_id != '' && $Date != '' && $towards != '')
{
 $amount=$gtotal;
...
if(isset($_POST['adult_name']) && isset($_POST['adult_gender']) && isset($_POST['adult_city']) && isset($_POST['adult_centre']))
{ 
...
$participent_type_child = "Child";
 ...
 include_once "../dbConnection.php";
    ...
    $sql = "INSERT INTO `event_registration` (`event_id`,`Fname`, `Lname`, `Mobile`, `Email`, `Address`, `PAN`, `Aadhar`, `Pin`, `Who_im`, `Transaction_id`, `Amount`, `Status`, `Transaction_start_time`, `Towards`,`receiptNumber`,`count_adult`,`count_yuva`,`count_child`,`dist`,`Payment_mode`,`Transaction_Number`,`paymenttype`,`userid`)
	  VALUES ('$eventId','$payerFirstName', '$payerLastName', '$payerContact', '$payerEmail', '$city', '$PANNo', '$aadhar', '$pin','$Who_im', '$trans_id', '$amount', '$Status','$Date','$towards','$receiptNumber','$adultcount','$yuvacount','$childcount','$dist','$paymode','$tranno','$paymenttype','$userid')";
        ...
        for ($i; $i < count($adultNameArray); $i++)
         { 
          $adult_coupon = '';
            ...
            $sql1 = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, coupon_number)  VALUES('". $eventId."','". $last_id."','".ucfirst($adultNameArray[$i])."','". ucfirst($adultGenderArray[$i])."','".ucfirst($adultCityArray[$i])."','".ucfirst($adultCentreArray[$i])."','".ucfirst($participent_type_adult)."','".ucfirst($adultBadgenoArray[$i])."')";
         ...
         { 
          ...
          if($towards == 'Foreign Sahaja Yogis paying via Indian Bank account') {
            ...
            $sqlyuva = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, coupon_number)  VALUES('". $eventId."','". $last_id."','".ucfirst($yuvaNameArray[$j])."','". ucfirst($yuvaGenderArray[$j])."','".ucfirst($yuvaCityArray[$j])."','".ucfirst($yuvaCentreArray[$j])."','".ucfirst($participent_type_yuva)."','".ucfirst($yuvaBadgenoArray[$j])."')";
         ...
         { 
          ...
          if($towards == 'Foreign Sahaja Yogis paying via Indian Bank account') {
            ...
            $sqlchild = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, coupon_number)  VALUES('". $eventId."','". $last_id."','".ucfirst($childNameArray[$k])."','". ucfirst($childGenderArray[$k])."','".ucfirst($childCityArray[$k])."','".ucfirst($childCentreArray[$k])."','".ucfirst($participent_type_child)."','".ucfirst($childBadgenoArray[$k])."')";
