...
<div class="col-md-12" style="padding-bottom: 10px;">
                ...
                <a href="event_contributionoffline.php" class="btn-sm btn btn-success">Offline Digital Partcipants MIS Report</a>
		...
		<!-- <input type="button" onclick="tableToExcel('testTable', 'W3C Example Table')" class="float-right" value="Export to Excel"> -->
            ...
            //   echo $sql;
              ...
              $sql_count = "SELECT Count(participant_type) from event_registration as ereg, participants as part".$queryCondition." and status='success' and participant_type = '$part_type' and ereg.event_registration_id = part.event_registration_id";
                        ...
                        $status = "Success";
                        $sql = "SELECT * from event_registration WHERE Status = 'Success' and paymenttype ='offline' and Payment_mode !='Cash' $cpeventtype ORDER BY event_registration_id desc";
                        $sql1 = "SELECT * from event_registration WHERE Status = 'Success' and paymenttype ='offline' and Payment_mode !='Cash' $cpeventtype ORDER BY event_registration_id desc";
                        $sql_amount = "SELECT Sum(Amount) from event_registration WHERE Status = 'Success' and paymenttype ='offline' and Payment_mode !='Cash' $cpeventtype ORDER BY event_registration_id desc";
                        $sql_count = "SELECT Count(participant_type) FROM `participants` as part, event_registration as reg where reg.Status='success' and reg.paymenttype ='offline' and reg.Payment_mode !='Cash' and reg.event_registration_id= part.event_registration_id";
