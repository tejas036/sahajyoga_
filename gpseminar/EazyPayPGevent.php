...
$optionalField= $PANNo.'||||||'.$towards.'|'.$city.'||2023-24';
...
if(isset($_POST['adult_name']) && isset($_POST['adult_gender']) && $_POST['adult_city'] && $_POST['adult_centre'] && $_POST['adult_age'])
{ 
...
$participent_type_child = "Child";
 ...
 session_start();
  ...
  $emailSession = $_SESSION['email'];
    ...
    $sql = "INSERT INTO `event_registration` (`event_id`,`Fname`, `Lname`, `Mobile`, `Email`, `Address`, `PAN`, `Aadhar`, `Pin`, `Who_im`, `Transaction_id`, `Amount`, `Status`, `Transaction_start_time`, `Towards`,`receiptNumber`,`count_adult`,`count_yuva`,`count_child`,`dist`,`terms_conditions`)
	  VALUES ('$eventId','$payerFirstName', '$payerLastName', '$payerContact', '$payerEmail', '$city', '$PANNo', '$aadhar', '$pin','$Who_im', '$trans_id', '$amount', '$Status','$Date','$towards','$receiptNumber','$adultcount','$yuvacount','$childcount','$dist','$terms_conditions')";
            ...
            $sqlCheckUser = "SELECT * FROM sahajyoga_user_registration WHERE email = '$emailSession'";
        ...
        for ($i; $i < count($adultNameArray); $i++)
         { 
          $adult_coupon = '';
            ...
            $sql1 = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, participant_age)  VALUES('". $eventId."','". $last_id."','".ucfirst($adultNameArray[$i])."','". ucfirst($adultGenderArray[$i])."','".ucfirst($adultCityArray[$i])."','".ucfirst($adultCentreArray[$i])."','".ucfirst($participent_type_adult)."','".ucfirst($adultAgeArray[$i])."')";
         ...
         { 
          ...
          if($towards == 'Only day Puja, Ganapatipule') {
            ...
            $sqlyuva = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, participant_age)  VALUES('". $eventId."','". $last_id."','".ucfirst($yuvaNameArray[$j])."','". ucfirst($yuvaGenderArray[$j])."','".ucfirst($yuvaCityArray[$j])."','".ucfirst($yuvaCentreArray[$j])."','".ucfirst($participent_type_yuva)."','".ucfirst($yuvaAgeArray[$j])."')";
         ...
         { 
          ...
          if($towards == 'Only day Puja, Ganapatipule') {
            ...
            $sqlchild = "INSERT INTO participants (event_id, event_registration_id, name, gender, city, centre, participant_type, participant_age)  VALUES('". $eventId."','". $last_id."','".ucfirst($childNameArray[$k])."','". ucfirst($childGenderArray[$k])."','".ucfirst($childCityArray[$k])."','".ucfirst($childCentreArray[$k])."','".ucfirst($participent_type_child)."','".ucfirst($childAgeArray[$k])."')";
